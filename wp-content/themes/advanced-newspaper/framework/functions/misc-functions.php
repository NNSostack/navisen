...
function gabfire_wp_title( $old_title, $sep, $sep_location ) {
	...
	if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		if ( is_post_type_archive('product') ) {
			$gabfire_title_text = woocommerce_page_title() . $sep . $title;
		} elseif(is_tax()) {
			$gabfire_title_text = sprintf( __( '%s ', 'gabfire' ), single_cat_title( '', false )) . $sep . $title;
		}
	}
 ...
 ******************************************************************** */
	function string_limit_words($word_limit) {
		global $post, $page;
		if( $post->post_excerpt ) {
			$word_limit = 999;
		}
		$words = explode(' ', wp_strip_all_tags(get_the_excerpt()), ($word_limit + 1));
 ...
 ******************************************************************** */
	...
	function gabfire_permalink() {
		echo '<span class="gabfire_meta gabfiremeta_readmore"><a href="'. get_the_permalink() . '" title="'; printf( esc_attr__( 'Permalink to %s', 'gabfire' ), the_title_attribute( 'echo=0' ) ); echo'" rel="bookmark">'; esc_attr_e('Read More', 'gabfire'); echo '</a></span>';
	}
	...
	function gabfire_postmeta($p_tag = true,$bydate = true,$comment = true,$permalink = true) {
