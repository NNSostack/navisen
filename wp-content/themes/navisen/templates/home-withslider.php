				...
				<?php if (intval(of_get_option('of_an_nrfea')) > 0 ) { ?>
					<div id="featured-slider" class="test">
						...
						<div class="cycle-slideshow" 
							...
							$do_not_duplicate = [];
							$count = 1;
							$of_an_nrfea = of_get_option('of_an_nrfea');
// 
							if($featured = get_option('navisen_featured')) {
								$disableslider = true;
								$args = array(
										'posts_per_page' => of_get_option('of_an_nrfea' , 5),
										'post__in' => explode(',',$featured),
										'orderby' => 'post__in'
								);
							
									
//							
							} elseif (of_get_option('of_an_featypeadvanced') == 'mrecent') {
								$args = array(
							...
							<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost" class="slide_item">
								...
								<div class="postcaption">
									...
									<?php  echo postinfo($post->ID); ?></h2>
									<p class="p-summary" itemprop="text"><?php echo navisen_string_limit_words(14); ?>&hellip;</p>
								</div>
...
//										$do_not_duplicate[] = $post->ID; 
						...
						<?php } 
							...
							$css_feature_nav_hidden = ' feature_nav_hidden ';
						} ?>
					</div><!-- #featured-slider -->
				<?php } ?>
			...
			<div id="below-slider" class="col-lg-12 col-md-12 col-sm-5 col-xs-12 <?php echo $css_feature_nav_hidden?>">
				<?php gabfire_dynamic_sidebar('primaryleft');?>
				
				<?php if (intval(of_get_option('of_an_nr1', 2)) > 0 ) {
					if($featured = get_option('navisen_left_list')) {
						$args = array(
							...
							?>
								...
								<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost" <?php post_class('featuredpost'); ?>>
									<div class="proportional_height">	
										...
										<h2 class="entry-title">
											<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf( esc_attr__( 'Permalink to %s', 'gabfire' ), the_title_attribute( 'echo=0' ) ); ?>" ><?php the_title(); ?></a>
											<?php echo postinfo($post->ID); ?>
										</h2>							
										
										<p class="p-summary" itemprop="text">
											<?php echo navisen_string_limit_words(); ?>
										</p>
									</div>
		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12">
				...
				<?php if (intval(of_get_option('of_an_nr2', 4)) > 0 ) { 
					$count = 1;
					if($featured = get_option('navisen_mid_list')) {
						$args = array(
					...
					while ($wp_query->have_posts()){ 
						...
						?>
							...
							<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost" <?php post_class($postclass); ?>>
								...
								?>				
								<p class="p-summary" itemprop="text"><?php echo navisen_string_limit_words(); ?></p>
								</div>
								<?php navisen_postmeta(true,true,true,false); ?>
		...
		<?php if (intval(of_get_option('of_an_nr3', 9)) > 0 ) { ?>
			...
			<div class="widget">
				<ul>	
					...
					?>	
						<li><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf( esc_attr__( 'Permalink to %s', 'gabfire' ), the_title_attribute( 'echo=0' ) ); ?>" ><?php the_title(); ?></a><?php echo postinfo($post->ID); ?></li>
...
for ($i=1; $i <= 5;$i++) {
	if(of_get_option('of_an_tabnr'.$i) !== '0' ) {
		$cat_count++;
		$single_catid = of_get_option('of_an_tabcat'.$i);
	}
}
...
if($cat_count == 1){
?>
		<p class="catname" style="padding: 0 15px;">
			<span><a data-toggle="tooltip" title="<?php printf(esc_attr__('View all posts filed under %1$s','gabfire'), get_cat_name($single_catid)); ?>"  href="<?php echo get_category_link($single_catid);?>"><?php echo get_cat_name($single_catid); ?></a></span>
		</p>
<?php 
} else {
?>
						...
						<div>
							...
							?>
								...
								<article itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost" <?php post_class($class); ?>>
									...
									<div class="proportional_height full_width">
										<h2 class="entry-title">
											<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf( esc_attr__( 'Permalink to %s', 'gabfire' ), the_title_attribute( 'echo=0' ) ); ?>" ><?php the_title(); ?></a>
											<?php echo postinfo($post->ID); ?>
										</h2>
										
										<p class="p-summary hidden-sm hidden-xs" itemprop="text"><?php echo navisen_string_limit_words(); ?></p>
									</div>
									<?php navisen_postmeta(true,true,false,false,false); ?>
